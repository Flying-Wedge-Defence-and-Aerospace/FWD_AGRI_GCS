...
WeakLinkInterfacePtr VehicleLinkManager::primaryLink() const {
    return _primaryLink;
}
...
void VehicleLinkManager::_commLostCheck(void)
{
    ...
    for (LinkInfo_t& linkInfo: _rgLinkInfo) {
        if (!linkInfo.commLost && !linkInfo.link->linkConfiguration()->isHighLatency() && linkInfo.heartbeatElapsedTimer.elapsed() > _heartbeatMaxElpasedMSecs) {
            //qDebug() << "Inside this block _commLostCheck";
...
void VehicleLinkManager::_addLink(LinkInterface* link)
{
    if (_containsLinkIndex(link) != -1) {
        ...
        //qDebug() << "_addLink CALLED for:" << link->linkConfiguration()->name();
        //qDebug() << "_rgLinkInfo count BEFORE append:" << _rgLinkInfo.count();
        ...
        //qDebug() << " Link appended. _rgLinkInfo count AFTER append:" << _rgLinkInfo.count();
        //qDebug() << " Stored link pointer:" << linkInfo.link.get();
...
void VehicleLinkManager::_linkDisconnected(void)
{
    ...
    qDebug() << "_linkDisconnected called ======> ";
    qDebug() << "_linkDisconnected linkCount" << _rgLinkInfo.count();
...
SharedLinkInterfacePtr VehicleLinkManager::_bestActivePrimaryLink(void)
{
    ...
    //qDebug() << " Entering _bestActivePrimaryLink. Total links:" << _rgLinkInfo.count();

    for (const LinkInfo_t& linkInfo : std::as_const(_rgLinkInfo)) {
        SharedLinkInterfacePtr link = linkInfo.link;
        QString linkName = link ? link->linkConfiguration()->name() : "null";
        //bool isHighLatency = link && link->linkConfiguration() ? link->linkConfiguration()->isHighLatency() : -1;

        // qDebug() << " Link:" << link.get()
        //          << "Name:" << linkName
        //          << "commLost:" << linkInfo.commLost
...
bool VehicleLinkManager::_updatePrimaryLink(void)
{
    ...
    if (linkIndex != -1 && !bestActivePrimaryLink) {
        ...
        if (bestActivePrimaryLink != primaryLink) {
            ...
            _primarySharedLink = bestActivePrimaryLink;
            emit primaryLinkChanged();
