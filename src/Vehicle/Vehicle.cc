...
void Vehicle::_handleVfrHud(mavlink_message_t& message)
{
    ...
    //qDebug() << "Air speed value" << vfrHud.airspeed;
    _groundSpeedFact.setRawValue(qIsNaN(vfrHud.groundspeed) ? 0 : vfrHud.groundspeed);
    //qDebug() << "Ground speed value" << vfrHud.groundspeed;
...
void Vehicle::_handleRCChannels(mavlink_message_t& message)
{
    ...
    for (int i=0; i<cMaxRcChannels; i++) {
        ...
        if (i < channels.chancount) {
            ...
            _latestRcValues[i] = (channelValue == UINT16_MAX ? UINT16_MAX : channelValue);
        } else {
            pwmValues[i] = -1;
            _latestRcValues[i] = UINT16_MAX;
        }
    }
...
{
     //qDebug() << "Sending MAVLink message ID:" << message.msgid << " to link:" << link->linkConfiguration()->name();
...
void Vehicle::_activeVehicleChanged(Vehicle *newActiveVehicle)
{
    ...
    if (_vehicleLinkManager) {
        //qDebug() << "Forcing primary link update on active vehicle switch";
        _vehicleLinkManager->_updatePrimaryLink();
    }
}
...
//     qDebug() << "Flight time (s):" << flightTime;
// }

void Vehicle::_updateFlightTime()
{
    double flightTime = static_cast<double>(_flightTimer.elapsed()) / 1000.0;
    _flightTimeFact.setRawValue(flightTime);
...
void Vehicle::sendJoystickDataThreadSafe(float roll, float pitch, float yaw, float thrust, quint16 buttons)
{
    ...
    // check the link type, if you want uncomment it
    // if (sharedLink) {
    //     qDebug() << "Link class:" << sharedLink->metaObject()->className();
    // }
...
void Vehicle::_sendOverrideValue(int channelNo, int overrideValue)
{
    mavlink_rc_channels_override_t rcOverride = {};
    rcOverride.target_system = id();
    rcOverride.target_component = defaultComponentId();

    uint16_t* channels[] = {
        &rcOverride.chan1_raw, &rcOverride.chan2_raw, &rcOverride.chan3_raw, &rcOverride.chan4_raw,
        &rcOverride.chan5_raw, &rcOverride.chan6_raw, &rcOverride.chan7_raw, &rcOverride.chan8_raw,
        &rcOverride.chan9_raw, &rcOverride.chan10_raw, &rcOverride.chan11_raw, &rcOverride.chan12_raw,
        &rcOverride.chan13_raw, &rcOverride.chan14_raw, &rcOverride.chan15_raw, &rcOverride.chan16_raw,
        &rcOverride.chan17_raw, &rcOverride.chan18_raw,
    };

    // for (int i = 0; i < cMaxRcChannels; ++i) {
    //     *channels[i] = _latestRcValues[i];
    // }

    if (channelNo >= 1 && channelNo <= cMaxRcChannels) {
        *channels[channelNo - 1] = overrideValue;
    } else {
        qDebug() << "Invalid override channel";
        return;
    }
...
void Vehicle::moveServo(int servoNumber, float pwmValue)
{
    // MAV_CMD_DO_SET_SERVO = 183
    sendMavCommand(
        defaultComponentId(),     // compId
        MAV_CMD_DO_SET_SERVO,     // command
        true,                     // showError
        static_cast<float>(servoNumber), // param1: servo number
...
// mavlink_rc_channels_override_t checkDecode;
// mavlink_msg_rc_channels_override_decode(&msg, &checkDecode);
