...
void RadioComponentController::_rcChannelsChanged(int channelCount, int pwmValues[Vehicle::cMaxRcChannels])
{
    ...
    for (int channel=0; channel<channelCount; channel++) {
        ...
        //qDebug() << "Channel value ====> " << channelValue;
        ...
        if (channelValue != -1) {
            ...
            //qDebug() << "Function name" << _rgChannelInfo[channel].function;
...
// void RadioComponentController::startOverrideStream(int channel, int value)
// {
//     _overrideChannel = channel;
//     _overrideValue = value;

//     qDebug() << "Override channel" << _overrideChannel;
//     qDebug() << "Override value" << _overrideValue;

//     if (!_rcOverrideTimer) {
//         _rcOverrideTimer = new QTimer(this);
//         connect(_rcOverrideTimer, &QTimer::timeout, this, &RadioComponentController::_sendOverride);
//     }

//     if (!_rcOverrideTimer->isActive()) {
//         _rcOverrideTimer->start(100);
//     }

//     // QTimer::singleShot(3000, this, &RadioComponentController::stopOverrideStream);
// }



//     void Vehicle::startOverride(int channelNo, int pwmValue)  From the vehicle.
// {
//     _overrideChannel = channelNo;
//     _overrideValue = pwmValue;

//     if(!_rcOverrideTimer)
//     {
//         _rcOverrideTimer = new QTimer(this);
//         connect(_rcOverrideTimer, &QTimer::timeout, this, [=]() {
//             _sendOverrideValue(_overrideChannel, _overrideValue);
//         });
...
// void RadioComponentController::_sendOverride()
// {
...
//     mavlink_rc_channels_override_t rcOverride = {};
//     rcOverride.target_system = vehicle->id();
//     rcOverride.target_component = vehicle->defaultComponentId();

//     qDebug() << "Vehicle ID: " << rcOverride.target_system;
//     qDebug() << "Target component: " << rcOverride.target_component;

//     for (int i = 0; i < Vehicle::cMaxRcChannels; ++i) {
//         (&rcOverride.chan1_raw)[i] = UINT16_MAX;
//     }

//     // if (_overrideChannel >= 1 && _overrideChannel <= Vehicle::cMaxRcChannels) {
//     //     (&rcOverride.chan1_raw)[_overrideChannel - 1] = _overrideValue;
//     // } else {
//     //     qDebug() << "Invalid override channel:" << _overrideChannel;
//     //     return;
//     // }

    // (&rcOverride.chan1_raw)[0] = 1410;
    // (&rcOverride.chan1_raw)[1] = 1681;
    // (&rcOverride.chan1_raw)[2] = 1378;
    // (&rcOverride.chan1_raw)[3] = 1765;
...
//     qDebug() << "System id: " << qgcApp()->toolbox()->mavlinkProtocol()->getSystemId();
//     qDebug() << "Component id: " << qgcApp()->toolbox()->mavlinkProtocol()->getComponentId();

//     qDebug() << "Sending RC override: Channel" << _overrideChannel << "Value" << _overrideValue;

//     const auto linkList = qgcApp()->toolbox()->linkManager()->links();
//     for (const SharedLinkInterfacePtr& sharedLink : linkList) {
//         LinkInterface* link = sharedLink.get();
//         if (link && link->isConnected()) {

//             qDebug() << "Sending override on link type:" << link->metaObject()->className();

//             qDebug() << "Override values array:";
//             for (int i = 0; i < Vehicle::cMaxRcChannels; ++i) {
//                 qDebug() << "Chan" << i+1 << "=>" << (&rcOverride.chan1_raw)[i];
//             }
...
//     mavlink_rc_channels_override_t rcOverride = {};
//     rcOverride.target_system = vehicle->id();
//     qDebug() << "Target system" << rcOverride.target_system;
//     rcOverride.target_component = vehicle->defaultComponentId();
//     qDebug() << "Target Component" << rcOverride.target_component;
...
//     mavlink_message_t msg;
//     mavlink_msg_rc_channels_override_encode(
//         /*1, // or use*/ qgcApp()->toolbox()->mavlinkProtocol()->getSystemId(),
//         qgcApp()->toolbox()->mavlinkProtocol()->getComponentId(),
//         &msg,
//         &rcOverride
//         );
...
//         LinkInterface* link = sharedLink.get();
//         if (link && link->isConnected()) {
//             QString linkType = link->metaObject()->className();
//             qDebug() << "Sending override on link type:" << linkType << "Is connected:" << link->isConnected();
//             vehicle->sendMessageOnLinkThreadSafe(link, msg);
//         }
//     }
// }
...
// void RadioComponentController::setCustomRCValues(int channelNo) {
//     qDebug() << "channel no " << channelNo;

//     Vehicle* _vehicle = qgcApp()->toolbox()->multiVehicleManager()->activeVehicle();

//     qDebug() << "Link count at click:" << _vehicle->vehicleLinkManager()->linkCount();
//     qDebug() << " Current primarySharedLink:" << _vehicle->vehicleLinkManager()->primarySharedLink().get();


//     if (!_vehicle) {
//         qWarning() << "vehicle pointer is null";
//         return;
//     }

//     auto* paraMgr = _vehicle->parameterManager();
//     if (!paraMgr->parametersReady()) {
//         qWarning() << "parameters not ready, try again later";
//         return;
//     }

//     const QString minName = QString("RC%1_MIN").arg(channelNo);
//     const QString maxName = QString("RC%1_MAX").arg(channelNo);
//     const QString trimName = QString("RC%1_TRIM").arg(channelNo);

//     Fact* minFact = getParameterFact(FactSystem::defaultComponentId, minName);
//     Fact* maxFact = getParameterFact(FactSystem::defaultComponentId, maxName);
//     Fact* trimFact = getParameterFact(FactSystem::defaultComponentId, trimName);

//     if (!minFact || !maxFact || !trimFact) {
//         qWarning() << "failed to get one of the parameter facts.";
//         return;
//     }

//     minFact->setRawValue(1100);
//     maxFact->setRawValue(1900);
//     trimFact->setRawValue(1600);

//     //if (channelNo == 6 || channelNo == 5) {
//     constexpr int overrideValue = 1400;
//         // static int modeIndex = 0;
//         // constexpr int modePWMValues[] = {1150, 1300, 1450, 1600, 1750, 1900};
//         // const int count = sizeof(modePWMValues) / sizeof(modePWMValues[0]);
//         // int pwm = modePWMValues[modeIndex];

//         qDebug() << "RC6 mode override:" << pwm;


//         _vehicle->vehicleLinkManager()->_updatePrimaryLink();
//         SharedLinkInterfacePtr sharedLink = _vehicle->vehicleLinkManager()->primarySharedLink();
//         qDebug() << "_primarySharedLink is:" << sharedLink.get();

//         if (!sharedLink) {
//             qWarning() << " No valid shared primary link.";
//             return;
//         }


//         mavlink_rc_channels_override_t rcOverride = {};
//         rcOverride.target_system = _vehicle->id();
//         qDebug() << "Target system" << rcOverride.target_system;
//         rcOverride.target_component = _vehicle->defaultComponentId();
//         qDebug() << "Target component" << rcOverride.target_component;
...
//         (&rcOverride.chan1_raw)[channelNo - 1] = overrideValue;

//         mavlink_message_t msg;
//         auto mavlinkProtocol = qgcApp()->toolbox()->mavlinkProtocol();
//         mavlink_msg_rc_channels_override_encode(
...
//         qDebug() << "Sending RC override...";
//         _vehicle->sendMessageOnLinkThreadSafe(sharedLink.get(), msg);
//         qDebug() << " RC override sent.";

//         //modeIndex = (modeIndex + 1) % count;
//    // }
// }
...
//     SharedLinkInterfacePtr link = vehicle->vehicleLinkManager()->primarySharedLink();
//     if (!link) return;

//     qDebug() << "Link class name:" << link->metaObject()->className();
//     //qDebug() << "Link URL:" << link->getName();
...
//     //qDebug() << "Target system" << rcOverride.target_system;  1
//     //qDebug() << "Target Component" << rcOverride.target_component;  1
...
//     qDebug() << "Sending override: chan" << _overrideChannel << "=" << _overrideValue;

//     mavlink_message_t msg;
//     mavlink_msg_rc_channels_override_encode(
//         /*qgcApp()->toolbox()->mavlinkProtocol()->getSystemId()*/1,
//         qgcApp()->toolbox()->mavlinkProtocol()->getComponentId(),
//         &msg, &rcOverride
//         );

//     //qDebug() << "System id: " << qgcApp()->toolbox()->mavlinkProtocol()->getSystemId();  1
//     //qDebug() << "Component id: " << qgcApp()->toolbox()->mavlinkProtocol()->getComponentId(); 190

//     //qDebug() << "Sending link" << link.get();  Sending link UDPLink(0x7eff1c2addc0)

//     qDebug() << "Link name:" << link.get() << " is connected:" << link->isConnected();  // Link name: UDPLink(0x7eff1c2addc0)  is connected: true

//     vehicle->sendMessageOnLinkThreadSafe(link.get(), msg);
// }
...
// void RadioComponentController::cycleFlightModeViaRC5()
// {
//     Vehicle* vehicle = qgcApp()->toolbox()->multiVehicleManager()->activeVehicle();

//     if (!vehicle) {
//         qWarning() << "Vehicle is null";
//         return;
//     }

//     if (!vehicle->parameterManager()->parametersReady()) {
//         qWarning() << "Parameters not ready yet";
//         return;
//     }

//     // Define custom mode values (match ArduCopter)
//     static int modeIndex = 0;
//     constexpr int flightModes[] = {
//         0,  // STABILIZE
//         1,  // ACRO
//         3,  // ALT_HOLD
//         4,  // AUTO
...
//         6   // LOITER
//     };
//     const int count = sizeof(flightModes) / sizeof(flightModes[0]);
//     int mode = flightModes[modeIndex];

//     qDebug() << "Requesting mode change to:" << mode;
...
//         static_cast<float>(mode) // use arduPilot custom mode
//         );
