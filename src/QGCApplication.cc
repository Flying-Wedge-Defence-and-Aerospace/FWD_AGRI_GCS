    ...
    , _runningUnitTests     (unitTesting)
{
    ...
    if (_runningUnitTests) {
        ...
        applicationName = QStringLiteral("%1 ").arg(QGC_APPLICATION_NAME);/*"FWDGCS";*/  /*Daily*/
        //applicationName = QStringLiteral("FWDGCS");
#else
...
{
    if (!_runningUnitTests) {
        qDebug() << "Versions" << _majorVersion << _minorVersion << _buildVersion;
...
{
    qDebug() << "Version String" << versionString << "Major version" << majorVersion << "Minor version" << minorVersion << "Built version" << buildVersion;
