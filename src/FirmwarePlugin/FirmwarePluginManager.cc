...
FirmwarePlugin* FirmwarePluginManager::firmwarePluginForAutopilot(MAV_AUTOPILOT firmwareType, MAV_TYPE vehicleType)
{
    ...
    if (factory) {
        // Only create plugin if not already created
        if (!_factoryFirmwarePlugin) {
            _factoryFirmwarePlugin = factory->firmwarePluginForAutopilot(firmwareType, vehicleType);
        }
        plugin = _factoryFirmwarePlugin;
    }
